...
permute_treatment <- function(data, adaptive=FALSE) {
  ...
  if (adaptive) {
    perm.data$tx = sample(data$tx)
  } 
  # classical randomization (this is used in Question 1)
  else {
    # re-randomize the outcomes of our sample
    perm.data$tx = sample(data$tx)
  }
  return(perm.data)
}
...
sim_one_trial <- function(data, adaptive=FALSE) {
  ...
  perm.data = permute_treatment(data, adaptive=FALSE)
...
perm_test <- function(data, adaptive=FALSE) {
  ...
  permutations = replicate(n.permutations, sim_one_trial(data, adaptive=FALSE))
  return(list(output=permutations, n=n.permutations))
}

## helper function to visualize permutation output
plot_perm_test <- function(perm.test, observed.stat, plot=TRUE) {
  ...
  gg = ggplot() + 
    ...
    geom_histogram(aes(simulated.summary.stats)) + 
...
# simulate a sampling distributions of the summary statistic using permutations
set.seed(1)
perm.test2 <- perm_test(data=adptv, adaptive=TRUE)
